...
See `settings.gradle.kts`'s own comment for the authoritative pipeline diagram (ANTLR → Analyzer →
Compiler/Simengine → Verilog → Vivado).
...
- **`brainstorming/`** — not a Gradle subproject, just design-notes markdown (`functions.md`,
  ...
  `parser.md`) plus canonical per-effort design/status docs in `brainstorming/claude/`
  (`netfpga-partial-synthesis.md`, `per-port-hierarchical-retiming.md` — each the entry point for its
  effort, with `todo.md` as the backing detail — and `netfpga-hls-kernels.md`, for building NetFPGA
  applications from HLS C++ alongside GAPL) plus **`brainstorming/todo.md`** and **`brainstorming/future-work.md`** — check these
