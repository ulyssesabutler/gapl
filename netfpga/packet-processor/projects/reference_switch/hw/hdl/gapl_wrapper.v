...
#(
    ...
    wire [TDATA_WIDTH - 1:0] gapl_order_in_tdata;
    wire [TDATA_WIDTH - 1:0] gapl_order_out_tdata;

    reverse_bytes #( .BYTES(TDATA_WIDTH / 8) ) gapl_order_in
    (
        .in(gapl_in_tdata),
        .out(gapl_order_in_tdata)
    );

    reverse_bytes #( .BYTES(TDATA_WIDTH / 8) ) gapl_order_out
    (
        .in(gapl_order_out_tdata),
        .out(gapl_out_tdata)
    );
