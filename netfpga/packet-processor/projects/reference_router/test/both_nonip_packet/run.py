...
for i in range(pkts_num):
	if isHW():
		for port in range(2):
			DA		= routerMAC[port]
			sent_pkt	= make_IP_pkt(dst_MAC=DA, src_MAC=SA, dst_IP=DST_IP, EtherType=EtherType, pkt_len=random.randint(60,1514))
...
# Read the counters
if isHW():
	rres1=nftest_regread_expect(SUME_OUTPUT_PORT_LOOKUP_0_PKT_SENT_TO_CPU_NON_IP_CNTR(), pkts_num*2)
	mres=[rres1]
else:
	nftest_regread_expect(SUME_OUTPUT_PORT_LOOKUP_0_PKT_SENT_TO_CPU_NON_IP_CNTR(), pkts_num)
	mres=[]
