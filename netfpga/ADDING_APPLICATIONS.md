...
netfpga/src/<app-name>/
    processor.gapl        # the application itself - shared by every variation below
...
`processor.gapl` and `test.properties` live one level *above* the variation directories on purpose:
...
`processor.gapl` must define a function named exactly `packet_body_processor`, taking and returning
...
Once `processor.gapl` exists, `./gradlew :compiler:installDist` followed by
`compiler/build/install/gapl/bin/gapl --flatten all -o /tmp/out.v netfpga/src/<app-name>/processor.gapl`
...
then place the result according to how your `processor.gapl` actually packs its output bits into the
...
Each subdirectory under `netfpga/src/<app-name>/` is a "variation" - the same `processor.gapl`
