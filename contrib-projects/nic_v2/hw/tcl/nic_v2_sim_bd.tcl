...
#####################################
source $::env(NF_DESIGN_DIR)/hw/tcl/nic_v2_defines.tcl
#source $::env(NF_DESIGN_DIR)/hw/tcl/export_registers.tcl

create_project -name ${design} -force -dir "$::env(NF_DESIGN_DIR)/hw/${proj_dir}" -part ${device}

file -force copy ${public_repo_dir}/ ${repo_dir}
...
# identifier
#set identifier_ip [create_bd_cell -type ip -vlnv xilinx.com:ip:blk_mem_gen:8.2 identifier_ip]
#set_property -dict [list CONFIG.Interface_Type {AXI4} CONFIG.AXI_Type {AXI4_Lite} CONFIG.AXI_Slave_Type {Memory_Slave} CONFIG.Use_AXI_ID {false} \
#CONFIG.Load_Init_File {true} CONFIG.Coe_File {$::env(NF_DESIGN_DIR)/hw/create_ip/id_rom16x32.coe} CONFIG.Fill_Remaining_Memory_Locations {true} \
#CONFIG.Remaining_Memory_Locations {DEADDEAD} CONFIG.Memory_Type {Simple_Dual_Port_RAM} CONFIG.Use_Byte_Write_Enable {true} \
#CONFIG.Byte_Size {8} CONFIG.Assume_Synchronous_Clk {true} CONFIG.Write_Width_A {32} CONFIG.Write_Depth_A {1024} CONFIG.Read_Width_A {32} \
#CONFIG.Operating_Mode_A {READ_FIRST} CONFIG.Write_Width_B {32} CONFIG.Read_Width_B {32} CONFIG.Operating_Mode_B {READ_FIRST} \
#CONFIG.Enable_B {Use_ENB_Pin} CONFIG.Register_PortA_Output_of_Memory_Primitives {false} CONFIG.Register_PortB_Output_of_Memory_Primitives {false} \
#CONFIG.Use_RSTB_Pin {true} CONFIG.Reset_Type {ASYNC} CONFIG.Port_A_Write_Rate {50} CONFIG.Port_B_Clock {100} \
#CONFIG.Port_B_Enable_Rate {100}] $identifier_ip
...
source $::env(NF_DESIGN_DIR)/hw/tcl/nic_v2_bd_port_sim.tcl
source $::env(NF_DESIGN_DIR)/hw/tcl/nic_v2_bd_connection_sim.tcl
