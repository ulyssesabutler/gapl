...
class NewHierarchicalMinimalRegisterSolver<G, N, E>(
    private val graphs: Collection<NewHierarchicalLeisersonCircuitGraph<G, N, E>>,
    private val expansionNodeFactory: () -> N,
    private val expansionEdgeValueFactory: () -> E,
) {
    data class SolveResult<G, N, E>(
        val retimedGraph: NewHierarchicalLeisersonCircuitGraph<G, N, E>,
    ...
    fun solveAll(targetClockPeriod: Int): Map<NewHierarchicalLeisersonCircuitGraph<G, N, E>, SolveResult<G, N, E>> {
        val results = mutableMapOf<NewHierarchicalLeisersonCircuitGraph<G, N, E>, SolveResult<G, N, E>>()
        val processed = mutableSetOf<NewHierarchicalLeisersonCircuitGraph<G, N, E>>()

        fun processGraph(graph: NewHierarchicalLeisersonCircuitGraph<G, N, E>) {
    ...
    private fun solveSingle(
        graph: NewHierarchicalLeisersonCircuitGraph<G, N, E>,
        childResults: Map<NewHierarchicalLeisersonCircuitGraph<G, N, E>, SolveResult<G, N, E>>,
        ...
        val flatNodeByHierarchicalNode = mutableMapOf<NewHierarchicalLeisersonCircuitGraph.Node<N>, WeightedGraph.Node<N>>()
        // Map from hierarchical child node to its expansion (keyed by Node<N> to avoid casts when looking up edge endpoints)
        val expansionByChildNode = mutableMapOf<NewHierarchicalLeisersonCircuitGraph.Node<N>, ChildExpansion<N>>()
        ...
        graph.nodes.forEach { hierarchicalNode ->
            ...
            when (hierarchicalNode) {
                ...
                is NewHierarchicalLeisersonCircuitGraph.VirtualNode<*> -> {
                    val flatNode = WeightedGraph.Node(0, nodeValue)
                    allFlatNodes.add(flatNode)
                    flatNodeByHierarchicalNode[hierarchicalNode] = flatNode
                }
                is NewHierarchicalLeisersonCircuitGraph.ChildGraphNode<*, *, *> -> { /* handled below */ }
            }
        }
        ...
        fun flatNodeFor(hierarchicalNode: NewHierarchicalLeisersonCircuitGraph.Node<N>, isSource: Boolean): WeightedGraph.Node<N> {
